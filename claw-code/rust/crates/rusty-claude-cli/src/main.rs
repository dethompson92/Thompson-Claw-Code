...
fn read_piped_stdin() -> Option<String> {
    if std::env::var("CLAW_SKIP_STDIN_CONTEXT").is_ok_and(|v| v == "1" || v == "true") {
        return None;
    }
    ...
    fn new(
        ...
        let provider_client = match provider_kind {
            ProviderKind::Anthropic => {
                ...
                if matches!(auth, api::AuthSource::None) {
                    return Err(api::ApiError::missing_credentials(
                        "Anthropic",
                        &["ANTHROPIC_AUTH_TOKEN", "ANTHROPIC_API_KEY"],
                    )
                    .into());
                }
                api::ProviderClient::Anthropic(
                    AnthropicClient::from_auth(auth)
                        .with_base_url(api::read_base_url())
                        .with_prompt_cache(PromptCache::new(session_id)),
                )
            }
            ProviderKind::OpenAi => api::ProviderClient::OpenAi(api::OpenAiCompatClient::from_env(
                api::OpenAiCompatConfig::openai(),
            )?),
            ProviderKind::Xai => api::ProviderClient::Xai(api::OpenAiCompatClient::from_env(
        ...
        Ok(Self {
            ...
            model: model.strip_prefix("openai/").unwrap_or(&model).to_string(),
            enable_tools,
            emit_output,
            allowed_tools,
            tool_registry,
...
UU conflicted.rs",
    ...
    fn managed_sessions_default_to_jsonl_and_resolve_legacy_json() {
        ...
        let legacy_path = super::sessions_dir().expect("dir").join("legacy.json");
